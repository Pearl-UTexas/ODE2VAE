...
class ODE2VAE(nn.Module):
    def __init__(self, n_filt=8, q=8):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(
                n_filt, n_filt * 2, kernel_size=5, stride=2, padding=(2, 2)
            ),  # 7,7
        ...
        self.bnn = BNN(
            2 * q, q, n_hid_layers=2, n_hidden=50, act="celu", layer_norm=True, bnn=True
        )
        ...
        self.decoder = nn.Sequential(
            UnFlatten(4),
            nn.ConvTranspose2d(
                h_dim // 16, n_filt * 8, kernel_size=3, stride=1, padding=(0, 0)
            ),
            nn.BatchNorm2d(n_filt * 8),
            nn.ReLU(),
            nn.ConvTranspose2d(
                n_filt * 8, n_filt * 4, kernel_size=5, stride=2, padding=(1, 1)
            ),
    ...
    def elbo(
        ...
        q = qz_m.shape[1] // 2
        # prior
        log_pzt = self.mvn.log_prob(
            zode_L.contiguous().view([L * N * T, 2 * q])
        )  # L*N*T
        ...
        lhood_L = torch.log(1e-3 + XrecL) * XL + torch.log(1e-3 + 1 - XrecL) * (
            1 - XL
        )  # L,N,T,nc,d,d
        ...
        if qz_enc_m is not None:  # instant encoding
            ...
            qenc_zt_ode = (
                Normal(mean_, std_)
                .log_prob(zode_L.contiguous().view(-1))
                .view([L, N, T, 2 * q])
            )
    ...
    def forward(self, X, Ndata, L=1, inst_enc=False, method="dopri5", dt=0.1):
        ...
        for l in range(L):
            ...
            oderhs = lambda t, vs: self.ode2vae_rhs(
                t, vs, f
        ...
        if inst_enc:
            ...
            lhood, kl_z, kl_w, inst_KL = self.elbo(
                qz0_m, qz0_logv, ztL, logpL, X, Xrec, Ndata, qz_enc_m, qz_enc_logv
            )
...
if __name__ == "__main__":
    ...
    for ep in range(Nepoch):
        L = (
            1 if ep < Nepoch // 2 else 5
        )  # increasing L as optimization proceeds is a good practice
        for i, local_batch in enumerate(trainset):
            minibatch = local_batch.to(device)
            elbo, lhood, kl_z, kl_w = ode2vae(
                minibatch, len(trainset), L=L, inst_enc=True, method="rk4"
            )[4:]
            tr_loss = -elbo
            optimizer.zero_grad()
            tr_loss.backward()
            optimizer.step()
            print(
                "Iter:{:<2d} lhood:{:8.2f}  kl_z:{:<8.2f}  kl_w:{:8.2f}".format(
                    i, lhood.item(), kl_z.item(), kl_w.item()
                )
            )
        with torch.set_grad_enabled(False):
            for test_batch in testset:
                ...
                break
        print(
            "Epoch:{:4d}/{:4d} tr_elbo:{:8.2f}  test_mse:{:5.3f}\n".format(
                ep, Nepoch, tr_loss.item(), test_mse.item()
            )
        )
