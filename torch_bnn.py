...
class BNN(nn.Module):
    def __init__(
        ...
        for i, (n_in, n_out) in enumerate(zip(layers_dim[:-1], layers_dim[1:])):
            self.weight_mus.append(
                Parameter(torch.Tensor(n_out, n_in), requires_grad=requires_grad)
            )
            self.bias_mus.append(
                Parameter(torch.Tensor(n_out), requires_grad=requires_grad)
            )
            self.acts.append(
                get_act(act) if i < n_hid_layers else get_act("linear")
            )  # no act. in final layer
            self.layer_norms.append(
                nn.LayerNorm(n_out)
                if layer_norm and i < n_hid_layers
                else nn.Identity()
            )
        if bnn:
            ...
            for i, (n_in, n_out) in enumerate(zip(layers_dim[:-1], layers_dim[1:])):
                self.weight_logsigs.append(
                    Parameter(torch.Tensor(n_out, n_in), requires_grad=requires_grad)
                )
                self.bias_logsigs.append(
                    Parameter(torch.Tensor(n_out), requires_grad=requires_grad)
                )
    ...
    def __sample_weights(self, mean=False):
        if self.bnn and not mean:
            weights = [
                weight_mu + torch.randn_like(weight_mu) * weight_sig.exp()
                for weight_mu, weight_sig in zip(self.weight_mus, self.weight_logsigs)
            ]
            biases = [
                bias_mu + torch.randn_like(bias_mu) * bias_sig.exp()
    ...
    def draw_f(self, mean=False):
        ...
        def f(x):
            for (weight, bias, act, norm) in zip(
                weights, biases, self.acts, self.layer_norms
            ):
    ...
    def kl(self):
        ...
        q = Normal(mus, sigs)
        N = Normal(
            torch.zeros(len(mus), device=mus.device),
