...
class ODE2VAE(object):
    ...
    def set_configs(self):
        ...
        if "mocap" in self.task:
            ...
            if self.is_enc_rnn:
                ...
                x0vs = tf.map_fn(
                    channel_stack,
                    (x0vs, tf.tile([self.amort_len], [tf.shape(x0vs)[0]])),
                    (tf.float32),
                )  # T-a_len, N, D, D, a_len
                x0ss = tf.map_fn(
                    channel_stack,
                    (x0ss, tf.tile([1], [tf.shape(x0ss)[0]])),
                    (tf.float32),
                )  # T-a_len, N, D, D, 1
                ...
                x0vs = tf.reshape(
                    x0vs, [len_inst_enc * N, D, D, self.amort_len]
                )  # (T-a_len)*N,D,D,amort_len
                x0ss = tf.reshape(
                    x0ss, [len_inst_enc * N, D, D, 1]
                )  # (T-a_len)*N,D,D,1
    ...
    def create_network(self):
        ...
        mvn = tfd.MultivariateNormalDiag(
            loc=tf.zeros(self.q), scale_diag=tf.ones(self.q)
        )

        def _sample_trajectory(xin):
            eps1 = tf.random_normal(
                tf.shape(self.v0_log_sigma_sq), 0, 1, dtype=tf.float32
            )
            v0 = tf.add(
                self.v0_mu, tf.multiply(tf.sqrt(tf.exp(self.v0_log_sigma_sq)), eps1)
            )
            eps2 = tf.random_normal(
                tf.shape(self.s0_log_sigma_sq), 0, 1, dtype=tf.float32
            )
            s0 = tf.add(
                self.s0_mu, tf.multiply(tf.sqrt(tf.exp(self.s0_log_sigma_sq)), eps2)
            )
            q0 = mvn.log_prob(eps1) + mvn.log_prob(eps2)
            q0 = tf.expand_dims(q0, 1)  # N,1
            vt, st, logpt = self.ode_density(
                v0, s0, q0, self.f, self.t
            )  # T, N, q [t0x0,t0x1,...t0xN,t1x0,t1x1,...]
            return vt, st, logpt

        vt_L, st_L, self.logpt_L = tf.map_fn(
            _sample_trajectory,
            tf.range(0, self.L),
        ...
        self.vt_mu, self.st_mu = self.ode(
            self.v0_mu, self.s0_mu, self.fmean, self.t
        )  # T, N, q
        self.st_mu = tf.reshape(self.st_mu, [-1, self.q], name="st_mu")  # T*N, q
        self.vt_mu = tf.reshape(self.vt_mu, [-1, self.q], name="vt_mu")  # T*N, q
        # Reconstruct
        self.x_rec_mu_L, self.x_rec_log_sigma_sq_L = self.decode(
            self.st_L, dec_fnc
        )  # T*N*L,D
    ...
    def enc_dense(self, x, nout, bn_list, scp="enc"):
        with tf.variable_scope(scp, reuse=tf.AUTO_REUSE):
            for i in range(self.Ne):
                ...
                x = tf.contrib.layers.fully_connected(
                    x, self.He, activation_fn=self.activation_fn
                )
            z0_mu = tf.contrib.layers.fully_connected(x, nout, activation_fn=None)
            z0_log_sig_sq = tf.contrib.layers.fully_connected(
                x, nout, activation_fn=None
    ...
    def create_loss(self):
        ...
        self.mean_reconstr_lhood = self.rec_lhood(
            self.x, self.x_rec_mu_mu, self.x_rec_log_sigma_sq_mu, True
        )
        self.reconstr_lhood = self.rec_lhood(
            self.x, self.x_rec_mu_L, self.x_rec_log_sigma_sq_L, False
        )
