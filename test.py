...
parser.add_argument(
    "--data_root", type=str, default="../neurips19/data", help="root of the data folder"
)
parser.add_argument(
    "--task", type=str, default="mocap_many", help="experiment to execute"
)
parser.add_argument("--ckpt", type=str, default="checkpoints", help="checkpoints file")
parser.add_argument(
...
def reconstruct(X, L=5, ts=None, fix_BNN=False, fix_enc=False):
    ...
    for l in range(L):
        ...
        Xrec = sess.run(
            Xrec_op_L, feed_dict={x: X, t: ts, is_train: False, Tss: X.shape[1]}
        )
    ...
    for j in range(N):
        for i in range(L):
            if q == 2:
                ...
                ax.plot(
                    zt[j, i, 0, :], zt[j, i, 1, :], zt[j, i, 2, :], "-", color=cols[i]
                )
                ax.scatter(
                    zt[j, i, 0, 0],
                    zt[j, i, 1, 0],
