...
if task == "bballs" or "mnist" in task:
    ext = "{:s}_q{:d}_inst{:d}_fopt{:d}_enc{:d}_dec{:d}".format(
        task, q, inst_enc_KL, f_opt, NF_enc, NF_dec
    )
elif "mocap" in task:
    ext = "{:s}_q{:d}_inst{:d}_fopt{:d}_He{:d}_Hf{:d}_Hd{:d}".format(
        task, q, inst_enc_KL, f_opt, He, Hf, Hd
    )
...
for epoch in range(num_epoch):
    ...
    sess.run(
        xtr_init_op,
        feed_dict={xfpl: dataset.train.x, xtpl: dataset.train.y, xbspl: batch_size},
    )
    while True:
        try:
    ...
    sess.run(
        xval_init_op,
        feed_dict={xfpl: dataset.val.x, xtpl: dataset.val.y, xbspl: xval_batch_size},
    )
    val_lhood = 0
    num_val_iter = 10
    ...
    if val_lhood > min_val_lhood:
        ...
        plot_reconstructions(
            ...
            ttr,
            show=False,
            fname="plots/{:s}/rec_tr_{:s}.png".format(task, ext),
        )
        plot_latent(
            zt,
            vae.q,
            vae.L,
            show=False,
            fname="plots/{:s}/latent_tr_{:s}.png".format(task, ext),
        )
        ...
        plot_reconstructions(
            ...
            tval,
            show=False,
            fname="plots/{:s}/rec_val_{:s}.png".format(task, ext),
        )
